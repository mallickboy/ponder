...
export default function Home() {
  ...
  // Update user state when Clerk user is loaded
  useEffect(() => {
    if (isClerkLoaded && clerkUser) {
      const newName = clerkUser.fullName || user.name;
      const newAvatar = clerkUser.imageUrl || user.avatar;
      
      
      // Update only name and avatar, preserve other user properties
      setUser(currentUser => ({
        ...currentUser,
        name: newName,
        avatar: newAvatar,
      ...
      console.log('Updating user with Clerk data:', {user});
    }
